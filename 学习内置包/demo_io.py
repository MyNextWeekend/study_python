...
def byte_io_test():
    ...
    bytes_file.write("中文".encode())  # 往流中写入数据

    print(
        f"构建字节完成: {bytes_file.getvalue()}"
    )  # 获取流中所有数据： b'hello world\xe4\xb8\xad\xe6\x96\x87'
    bytes_file.seek(2)  # 设置指针偏移位置
    print(
        f"从指针位置读取数据:{bytes_file.read()}"
    )  # b'llo world\xe4\xb8\xad\xe6\x96\x87'
...
def string_io_test():
    ...
    print(
        f"构建字符串完成: {cache_file.getvalue()}"
    )  # 获取流中所有数据：hello world中文
