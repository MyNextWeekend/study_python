...
如果 python中安装的第三方库是OpenCV4.1.0.25，需要降级处理：
...
if __name__ == '__main__':
    img = cv2.imread('pictures/chepai.jpg', cv2.IMREAD_COLOR)
    # 预处理图像
    rect, afterimg = find_license(img)

    # 框出车牌号
    cv2.rectangle(afterimg, (rect[0], rect[1]), (rect[2], rect[3]), (0, 255, 0), 2)
    cv2.imshow('step1 afterimg', afterimg)

    # 分割车牌与背景
    cutimg = cut_license(afterimg, rect)
    cv2.imshow('step2 cutimg', cutimg)

    # 二值化生成黑白图
    thresh = deal_license(cutimg)
    cv2.imshow('step3 thresh', thresh)
    cv2.waitKey(0)
